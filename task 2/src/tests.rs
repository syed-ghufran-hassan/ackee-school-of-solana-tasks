...
mod calculator_tests {
    ...
    fn random_inputs_calculator() {
        ...
        for _ in 0..50_000 {
            let x_in = rng.random::<i64>();
            let y_in = rng.random::<i64>();
...
mod shapes_tests {
    ...
    fn random_inputs_shapes() {
        let mut rng = rand::rng();
        for _ in 0..50_000 {
            let a_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
            let a_magnitude: f64 = rng.random::<f64>();
            let width_in = a_sign * a_magnitude * f32::MAX as f64;

            let b_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
            let b_magnitude: f64 = rng.random::<f64>();
            let height_in = b_sign * b_magnitude * f32::MAX as f64;

            let r_sign = if rng.random::<bool>() { 1.0 } else { -1.0 };
            let r_magnitude: f64 = rng.random::<f64>();
